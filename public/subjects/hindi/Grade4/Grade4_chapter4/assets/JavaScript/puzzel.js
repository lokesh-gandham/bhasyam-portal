...
const gameData = {
    ...
    answers: [
        ...
        { number: 2, word: "आशीर्वाद" },

        { number: 3, word: "प्रणाम" },

        { number: 4, word: "सभा" },

        { number: 5, word: "राजकवि" },
        ...
        function selectLetter(cell, letter) {
            ...
            if (currentSelection.length > 0) {
                ...
                const lastIndex =
                    cells.indexOf(lastCell);

                const lastRow =
                    Math.floor(lastIndex / cols);

                const lastCol =
                    lastIndex % cols;

                const isNextHorizontal =
                    currentRow === lastRow &&
                    currentCol === lastCol + 1;

                const isNextVertical =
                    currentCol === lastCol &&
...
const lastIndex = cells.indexOf(
    currentSelection[currentSelection.length - 1].element
);
...
currentSelection.forEach((x, index) => {
    ...
    if (solvedWords === totalWords) {
        ...
        setTimeout(showFinal, 1000);
            ...
            const possible =
                gameData.answers.some(a =>
                    a.word.startsWith(formedWord)
                );
