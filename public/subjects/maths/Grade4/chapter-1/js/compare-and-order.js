...
function formQ(digits){
  ...
  return {
    ...
    render(stage, ready, saved){
      ...
      smPicked = saved ? saved.smPicked.slice() : [];
      grPicked = saved ? saved.grPicked.slice() : [];
      ...
      chipRow.className = 'form-chip-row';
      wrap.appendChild(chipRow);
      chipRow.appendChild(sm.chips);
      chipRow.appendChild(gr.chips);

      const undo = document.createElement('button');
      undo.className = 'form-undo';
      undo.type = 'button';
      undo.textContent = 'Undo';
      chipRow.appendChild(undo);
