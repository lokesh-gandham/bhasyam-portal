  ...
  function build(stage, saved, locked){
    ...
    wordField.addEventListener('input', () => {
      if(wordField.value.length > 0){
        wordField.value = wordField.value.charAt(0).toUpperCase() + wordField.value.slice(1);
      }
      gate();
    });
  }
...
function periodsQ(chunks){
  ...
  function paint(){
    ...
    const done = PERIODS.every(p => placed[p]);
    outEl.textContent = done ? PERIODS.map(p => placed[p]).join(',') : '';
    outEl.classList.toggle('formed', done);
  }
