...
(function(){
  ...
  function showPopout(type, msg, onClose){
    ...
    const icon = isOk ? '🎉' : '💡';
    const label = isOk ? 'Correct Answer' : (isHint ? 'Hint' : 'Keep Learning');
    const title = isOk ? 'Brilliant!' : (isHint ? 'Need a clue?' : 'Almost There!');
    const cls = isOk ? 'correct' : (isHint ? 'hint' : 'wrong');
    ...
    if(isOk){ soundCorrect(); confetti({ count:40 }); }
    else if(!isHint) soundWrong();

    const autoCloseTime = isHint ? 3500 : 2000;
  ...
  /* Grows a textarea to fit its content so a long typed answer is never
     scrolled or clipped — it just wraps onto more lines. */
  ...
  window.Quiz = {
    ...
    start(cfg){
      ...
      function paint(){
        ...
        els.hintBtn.style.display = q.hint ? '' : 'none';
        els.hintBtn.onclick = q.hint ? () => showHint(q.hint) : null;
        els.stage.innerHTML = '';
      ...
      function reveal(){
        ...
        function afterDelay(){
          ...
          if(state.i === total - 1){
            showPopout('correct', res.msg, ()=>{ finish(); });
          } else {
            showPopout('correct', res.msg, ()=>{ paint(); });
          }
        }
      ...
      els.next.addEventListener('click', ()=>{
        if(state.answeredResults[state.i] === null) return;
        if(state.i < total - 1){
          state.i++;
          paint();
        } else {
    ...
    tubes(stage, names, {counts = null, editable = true, onChange = null, maxVisible = 9} = {}){
      const n = names.length;
      const unit = i => Math.pow(10, n - 1 - i);
      const c = counts ? counts.slice() : names.map(() => 0);

      const wrap = document.createElement('div');
      wrap.className = 'tubes-wrap' + (maxVisible >= 9 ? ' tubes-all' : '');
