...
const COUNTS = [5,3,1,7,4,2];
const NUMERALS = '531742';
const WORDS = 'Five lakh thirty one thousand seven hundred forty two';
...
function readQ(){
  ...
  function answers(stage, num, word, locked){
    ...
    rail.innerHTML = `
      ...
      <textarea class="name-input" rows="1"
             placeholder="Write it in words" autocomplete="off" ${locked ? 'disabled' : ''}>${word}</textarea>`;
    stage.appendChild(rail);
    const wordEl = rail.querySelector('.name-input');
    Quiz.autoGrow(wordEl);
    return [rail.querySelector('.entry'), wordEl];
  }
  ...
  return {
    ...
    render(stage, ready, saved){
      ...
      [numField, wordField] = answers(stage, vals.num, capName(vals.word), !!saved);
      if(saved) return;
      const check = () => {
        if(numField.value.trim() && wordField.value.trim().length > 3) ready();
      };
      numField.addEventListener('input', check);
      wordField.addEventListener('input', () => { Quiz.autoGrow(wordField); check(); });
      setTimeout(() => numField.focus(), 30);
    },
    renderLocked(stage, saved){
      Quiz.tubes(stage, NAMES, { counts: COUNTS, editable:false });
      answers(stage, saved ? saved.num : NUMERALS, capName(saved ? saved.word : WORDS), true);
    },
    check(){
      ...
      const pretty = capName(wordField.value);
      wordField.value = pretty;
      return { ok:true, answer:{ num:numField.value.trim(), word:pretty },
               msg:`Perfect — ${NUMERALS} is ${WORDS}.` };
    }
  };
}
